...
**Solution:** Show first using `head`:
...
for i in Darcy Elizabeth Jane Lydia
  do 
  echo "$i" 
  cat /home/edwardmjyu/biostat-m280-2019-winter/hw1/pride_and_prejudice.txt | grep -c $i 
done 
    ```
| Name | # of References | 
|:------|:-----|
...
**Solution:** middle.sh takes in a text file and from the first 20 lines displays the last 5 lines of the 20 lines, and more generally, the code will do the same for any numerical combination. The meaning of `"$1"`, `"$2"`, and `"$3"` indexes the order of arguments required for middle.sh. '"$1"' refers to the file name, '"$2"' is the number of lines head displays, and '"$3"' is the number of lines tail finally displays from the initial head block of lines. The first line of the shell script determines which type of shell should run the program. In our case, the shell type is sh. 
...
**Solution:**
    ...
    ```{bash}
    Rscript runSim.R n=100 dist=\"gaussian\" seed=3 
    ```
...
**Solution:** Here is my 'autoSim.R' code:
  ...
  Let's run simulations with various 'n' and 'dist':
    ```{bash}
    Rscript autoSim.R
    ```
