...
latix %>% 
  ...
  arrange(desc(n))
```
**Solution**: The barchart shows that most tickets are issued on `Monday` it most likely to get a ticket on `Monday` and least likely to get a ticket on `Saturday`. 
...
latix %>% 
  ...
  arrange(desc(n))
```
**Solution**: The barchart shows that it most likely to get a ticket on day `13` and least likely to get a ticket on day `31`. This is most likely due to there being the least number of months with `31` days. 
...
latix %>% 
  ...
  arrange(desc(n))
```
**Solution**: The barchart shows that it most likely to get a ticket on `August` and least likely to get a ticket on `Februrary`. This is most likely due to there being the least days in `February`
