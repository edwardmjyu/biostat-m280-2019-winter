...
Below is a link to my shiny new app:  <https://edwardmjyu.shinyapps.io/lapayroll_shiny/>
...
```
**Solution**: There are 7656,362 distinct tickets, SQL removes any `NA` data.
...
latix %>% 
  ...
  arrange(desc(n))
```
**Solution**: The bar chart shows that most tickets are issued on `Monday` it most likely to get a ticket on `Monday` and least likely to get a ticket on `Saturday`. 
...
latix %>% 
  ...
  arrange(desc(n))
```
**Solution**: The bar chart shows that it most likely to get a ticket on `August` and least likely to get a ticket on `Februrary`. This is most likely due to there being the least days in `February`
...
dbGetQuery(con, car) 
```
**Solution**: The make that received the most citations was the Toyota with a total of `1,270,804` citations.
